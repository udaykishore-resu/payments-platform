...
output "nat_egress_ips" {
  description = "NAT public IPs, allowlisted by the gateway sandbox accounts."
  value       = module.network.nat_public_ips
}

output "eks_cluster" {
  description = "Cluster coordinates for the CI pipeline."
  value = {
...
output "aurora" {
  description = "Aurora connection targets. No credential: the application uses IAM tokens."
  value = {
...
output "msk_bootstrap_servers" {
  description = "SASL/IAM bootstrap servers."
  value       = module.msk.bootstrap_brokers_sasl_iam
}

output "redis_endpoint" {
  description = "Redis primary endpoint. Cluster mode is off in staging, so this is the primary endpoint, not a configuration endpoint - the Helm values select the client mode from this."
...
output "secret_arns" {
  description = "Platform secret ARNs - names, not values."
  value       = module.secrets.secret_arns
}

output "secrets_path_prefix" {
  description = "Secret path prefix for the External Secrets ClusterSecretStore."
...
output "dr_control" {
  description = "Fencing table. Single-region here, but the code path is exercised on every request."
  value = {
