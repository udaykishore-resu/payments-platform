...
| Cliff never exercised, so its behaviour is broken when first needed | High if untested | High | `tests/chaos/control_plane_loss_test.go` runs past the cliff on every CI chaos run and asserts both sides of the asymmetry; a staging drill runs quarterly | Chaos test result; drill record |
