...
|---|---|---|
| **B1 — No shared tables** | A table has exactly one owning context. No other context reads it, writes it, or joins to it — not even `SELECT`. The owner may not grant access to another context's database role. | Per-context Postgres roles with `GRANT` only on owned tables. `scripts/check-table-ownership.sh` diffs `information_schema.role_table_grants` against `docs/spec/05-bounded-contexts.md` §4 and fails CI on drift. |
