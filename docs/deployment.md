...
| Secrets | Never in env vars or ConfigMaps. External Secrets Operator projects them as files from Secrets Manager; the app reads them as `Secret[T]` (§17.2) and refreshes on rotation without a restart |
...
| Dashboards | **Amazon Managed Grafana**, SSO-federated | Datasources: AMP, Tempo, Loki, CloudWatch. Dashboards as code, provisioned by ArgoCD from `deployments/observability/dashboards` |
...
Splitting them means: a production deploy is a commit to the GitOps repo that changes one image digest, reviewable in ten seconds; the application repo's history is code, not deployment noise; and CI's write access to production is confined to "may open a PR that changes an image digest in `environments/prod/image-versions.yaml`" — it holds no cluster credentials at all.
...
spec:
  ...
  strategy:
    canary:
      ...
      trafficRouting:
        alb: { ingress: payment-api, servicePort: 8080 }
      maxSurge: "25%"
...
| 4 | **Logging/redaction check** | `scripts/check-logging.sh` (`%+v` on request types, non-`Secret[T]` credentials, unregistered log fields) | Any finding fails |
| 5 | **Metrics cardinality lint** | `scripts/metrics-lint.sh` (`observability.md` §3.3) | Forbidden label, undeclared metric, orphan metric |
...
| 22 | **SLO budget gate** | `scripts/slo-gate.sh` (`observability.md` §4.4) | Freeze tier blocks non-exempt PRs |
