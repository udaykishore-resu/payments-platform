...
func TestTheDocumentedMinimumIsAccepted(t *testing.T) {
	...
	// The lease margin is the property that keeps two workers off one instance. Three times the
	// heartbeat tolerates two consecutive misses — a GC pause and a slow query — without
	// tolerating a wedged worker.
	if cfg.HeartbeatInterval*3 > cfg.LeaseDuration {
		t.Errorf("the default heartbeat %s cannot safely hold the default lease %s",
			cfg.HeartbeatInterval, cfg.LeaseDuration)
	}
	if cfg.WorkflowName == "" {
		t.Error("PP_WORKFLOW_NAME has no default; the worker would drive no definition")
	}
}
